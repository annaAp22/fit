...
@section('main')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <ul class="breadcrumb">
            ...
            <li class="active">Добавление партнера</li>
        </ul><!-- /.breadcrumb -->
    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-xs-12">
                <!-- PAGE CONTENT BEGINS -->
                <form class="form-horizontal" role="form" action="{{route('admin.partners.store')}}" method="POST">
                    <input name="_token" type="hidden" value="{{csrf_token()}}">

                    <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="form-field-20"> Пользователь </label>
                        <div class="col-sm-9">
                            <select name="user_id" id="form-field-20">
                                @foreach($users as $user)
                                    <option value="{{$user->id}}" @if($user->id == $users[0]->id)selected="selected"@endif>{{$user->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
